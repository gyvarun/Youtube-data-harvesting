...
# [Dash board libraries - streamlit, seaborn, matplotlib]
...
tab1, tab2, tab3 = st.tabs(["Data collection", "Data Migration", "Channel Analysis"])
with tab1:
   st.header(':green[Data collection]')
   channel_id = st.text_input('**Enter Channel_id**')
   st.write('''Click on **:green['Get Channel Data']** to collect the channel data .''')
   Get_data = st.button(':violet[**Get Channel Data**]')
...
# Define Session state to Get data button
   ...
   if Get_data or st.session_state.Get_state:
        ...
        def get_video_comments(youtube, video_id, max_comments=2):
        ...
        st.write(f"Updated document id: {upload.upserted_id if upload.upserted_id else upload.modified_count}:sunglasses: :sunglasses:")

        # Close the connection
        client.close()

# Data Migrate zone (Store data to MySQL)
...
with tab2:
    ...
    document_name = st.selectbox('**Select Channel**', options = document_names, key='document_names')
    st.write('''Click on **:green['Migrate Channel Data']** to process the collected channel data.''')
    Migrate = st.button(':violet[**Migrate Channel Data**]')
    ...
    Check_channel = st.toggle('**Show available channels for analysis**')
    ...
    if Check_channel:
        ...
        df_at_sql = pd.DataFrame(channel_names_fromsql, columns=['Available Channels']).reset_index(drop=True)
...
# Channel Analysis
with tab3:
    st.header(':green[Channel Analysis]')
# Questions
    # Selectbox creation
